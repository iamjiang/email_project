...
def main(args,df,device):
    ...
    model_path=os.path.join("/opt/omniai/work/instance1/jupyter/", "v2_new_email/fine-tune-LM",output_path)
    ...
    df["fill_masked_email"]=masked_email   
...
#     df['fill_masked_email'] = df['fill_masked_email'].progress_apply(remove_non_english)
    df['fill_masked_email'] = df['fill_masked_email'].progress_apply(clean_re)
        
    if not os.path.exists(args.output_dir):
        os.makedirs(args.output_dir)
        
    df.to_pickle(os.path.join(output_dir,"fill_masked_email_pickle"))

if __name__=="__main__":
    parser = argparse.ArgumentParser(description='fill_masked_email to augment complaint emails')
