...
class Custom_Dataset_Class(Dataset):
    def __init__(self, df, tokenizer, feature_name, target, max_len=512, overlap_len=64):

        self.tokenizer = tokenizer
        self.max_len = max_len
        self.overlap_len = overlap_len

        self.data, self.label=df[feature_name].values,  df[target].values
    ...
    def __getitem__(self, idx):
        ...
        targets_list=[targets]*len(input_ids_list)
        
        return dict(
            input_ids=input_ids_list,
            attention_mask=attention_mask_list,
            labels=targets_list,
            chunk_len=[torch.tensor(len(targets_list), dtype=torch.long)]
        )
    ...
    def collate_fn(self,batch):
        ...
        chunk_len=torch.stack([torch.tensor(x["chunk_len"]) for x in batch])
        
#         for idx, (inp, att) in enumerate(zip(input_ids,attention_mask)):
            
#             pad_token_id=self.tokenizer.pad_token_id
#             keep_mask = inp.ne(pad_token_id).any(dim=0)
#             inp=inp[:, keep_mask]
#             att=att[:, keep_mask]
#             input_ids[idx]=inp
#             attention_mask[idx]=att
