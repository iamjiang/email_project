...
class Loader_Creation(Dataset):
    ...
    def preprocess_function(self,examples):
        ...
        target_max_length = max([len(self.tokenizer(class_label)["input_ids"]) for class_label in self.text_label])
        remaining=self.model_max_length-len(indexed_prompt_v1)-len(indexed_prompt_v2)-target_max_length
        ...
        for i in range(batch_size):
            ...
            * (self.model_max_length - len(sample_input_ids)) + sample_input_ids
            ...
            labels["input_ids"][i] = [-100] * (self.model_max_length  - len(sample_input_ids)) + label_input_ids
            ...
            labels["input_ids"][i] = torch.tensor(labels["input_ids"][i][:self.model_max_length])
    ...
    def __getitem__(self,index):
        ...
        _target = self.dataset.loc[index]["labels"].squeeze()
