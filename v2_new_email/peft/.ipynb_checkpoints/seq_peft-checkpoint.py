import sys
sys.path.append('/opt/omniai/work/instance1/jupyter/v2_new_email/peft/src')
sys.path.append('/opt/omniai/work/instance1/jupyter/v2_new_email/peft')
...
def under_sampling(df,target, seed, negative_positive_ratio):
    ...
    feedback=df["is_feedback"].values.squeeze()
    IDX=np.arange(LABEL.shape[0])
    positive_idx=IDX[LABEL==1]
    negative_idx_1=np.random.choice(IDX[(LABEL==0) & (feedback=="N")],size=(len(positive_idx)*negative_positive_ratio))
    negative_idx_2=IDX[feedback=="Y"] ### include feedback=1 in the negative sample
    
    _idx=np.concatenate([positive_idx, negative_idx_1, negative_idx_2])
...
def main(args,train_data, val_data, device):
    ...
    if any(k in args.model_name.split("-") for k in ["gpt", "opt", "bloom"]):
        padding_side = "left"
    else:
        padding_side = "right"
    ...
    train_module=Loader_Creation(hf_data["train"], tokenizer,"preprocessed_email")
    val_module=Loader_Creation(hf_data["validation"], tokenizer,"preprocessed_email")
...
if __name__=="__main__":
    parser = argparse.ArgumentParser(description='Parameters Efficient Fine Tuning')
