...
def main(args,train_data, val_data, test_data, device):
    ...
    if args.customized_tokenizer:
        model_name=os.path.join("/opt/omniai/work/instance1/jupyter/", "email-complaints/fine-tune-LM",model_path,"JPMC-email-tokenizer")
    else:
...
#         accelerator.print(f"\n** avg_loss : {epoch_loss:.2f}, time :~ {(time.time()-t0)//60} min ({time.time()-t0 :.2f} sec)***\n")
    ...
    best_threshold=utils.find_optimal_threshold(val_target.squeeze(), val_pred[:,1].squeeze(), min_recall=0.85, pos_label=False)
