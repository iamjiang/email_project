...
def catboost_traning(x_train,y_train,x_val,y_val,x_test,y_test,cat_features_names = ["service_feature","front_office","negative_word"],target="target_variable"):
...
def lightgbm_training(x_train,y_train,x_val,y_val,x_test,y_test,cat_features_names = ["service_feature", "front_office", "negative_word"],target="target_variable"):
...
def xgboost_training(x_train,y_train,x_val,y_val,x_test,y_test,cat_features_names = ["service_feature", "front_office", "negative_word"],target="target_variable"):
...
def randomforest_training(x_train,y_train,x_val,y_val,x_test,y_test,cat_features_names = ["service_feature", "front_office", "negative_word"]):
...
if __name__=="__main__":
    ...
    parser.add_argument("--test_negative_positive_ratio",  type=int,default=20,help="Undersampling negative vs position ratio in test set")
    ...
    if args.val_test_undersampling:
        df_var=utils.under_sampling(df_var,"target_variable",seed=args.seed, negative_positive_ratio=args.test_negative_positive_ratio)
        df_test=utils.under_sampling(df_test,"target_variable",seed=args.seed, negative_positive_ratio=args.test_negative_positive_ratio)

    y_train=df_train.loc[:,["target_variable"]]
    x_train=df_train.drop(["target_variable"],axis=1)

    y_val=df_val.loc[:,["target_variable"]]
    x_val=df_val.drop(["target_variable"],axis=1)

    y_test=df_test.loc[:,["target_variable"]]
    x_test=df_test.drop(["target_variable"],axis=1)
    
    cat_features_names = ["service_feature","front_office","negative_word"]
    
    if args.model_name=="catboost":
        clf_model, train_pred, val_pred, test_pred=catboost_traning(x_train,y_train,x_val,y_val,x_test,y_test,cat_features_names = cat_features_names,target="target_variable")
    elif args.model_name=="lightgbm":
        lgb_model, train_pred, val_pred, test_pred=lightgbm_training(x_train,y_train,x_val,y_val,x_test,y_test,cat_features_names = cat_features_names,target="target_variable")
    elif args.model_name=="xgboost":
        xgb_model, train_pred, val_pred, test_pred=xgboost_training(x_train,y_train,x_val,y_val,x_test,y_test,cat_features_names = cat_features_names,target="target_variable")
        
    elif args.model_name=="randomforest":
        rf_model, train_pred, val_pred, test_pred=randomforest_training(x_train,y_train,x_val,y_val,x_test,y_test,cat_features_names = cat_features_names)
    else:
        pass
